...
export default function App() {
  ...
  const postNewFriend = newFriend => {
    ...
    axios.post('https//buddies.com/api/friends', newFriend)
    .then(res => {
      setFriends(res.data);
    }).catch(err => console.error(err));
  }
  ...
  const formSubmit = () => {
    const newFriend = {
      ...
      // 🔥 STEP 7- WHAT ABOUT HOBBIES?         ^^ If hobby is true, it'll be added to a new array. True hobbies are kept in state ^^
    }
